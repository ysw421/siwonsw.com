  ...
  return (
    <Paper title='Optimization'>
      ...
      <p>
        ...
        표현하겠습니다 (다수의 문헌은 step-size를 learning rate라 명명하지만,
        '얼마 만큼 울직일지 설정함'의 의미를 보다 명확히 표현하기 위해
        ...
        <InlineMath math='\gamma < 0' />일 경우 경사 하강법식은 local minimum이
        아닌 local maximum에 수렴하며 이 경우 경사 상승법이라 부릅니다. 경사
        하강법의 시간별 위치(index)를 <InlineMath math='x_t' />로 표현하며 첫
        index 값인 <InlineMath math='x_0' />은 임의의 수로 설정합니다. 1차
        최적화 알고리듬(output은 언제나 <InlineMath math='\mathbb{R}' />
      ...
      <div>
        ...
        )보다 보다 느리게 수렴합니다. step-size가 어느 정도 이상인 경우(
        <Button
          className='py-1 mx-1'
          onClick={() => {
            setGd_A(width / 4 - 100);
        ...
        )에는 곡선을 '지그재그' 모양으로 넘어가며 이동합니다. 비단 step-size가
        너무 큰 경우(
        <Button
          className='py-1 mx-1'
          onClick={() => {
        ...
        되었다고 표현합니다. Case 6의 경우, step-size를 인위적으로 범위를
        벗어나는 큰 값으로 지정하였습니다.
        <Box>
          ⚠️ 필자는 위 object를 제작하며 안정성(오류가 발생하지 않는)을 위해
          다양한 제약한 조건을 추가하였습니다(함수의 좌측과 우측은 2차 함수로
          구성된 방면, 중앙은 3차 함수로 구성 되었습니다. Case 6에서 start
      ...
      <div>
        ...
        최적화가 느리게 진행됩니다(input의 차원을 하나 늘린다면 해당 경사
        하강법의 문제점에 대하여 더 좋은 시각화가 가능합니다. 시간이 남는다면
        시각화를 진행해 보겠습니다). momentum 기법의 경우 '공을 굴리는 것'과
        같이 관성을 이용합니다. 프로그래밍에서 메모리를 소비할 경우 연산 속도는
        빨라지는데, momentum 또한 같습니다.
      </div>
      <div className='w-4/5 m-auto my-8 aspect-video'>
        <MomentumSVG />
      </div>
